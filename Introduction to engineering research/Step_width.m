...
load('Day6QS.mat'); %load the trial you want to analyze
...
GRF_1foot = GRF/2; % per foot/side, since GRF is measured on a split belt treadmill
...
load('Day6ZT2.mat'); %load the trial you want to analyze
...
%% filter outliers
fL_CoPx = filloutliers(L_CoPx, 'nearest');
fR_CoPx = filloutliers(R_CoPx, 'nearest');
...
for k = 1:length(Lheel_strikes1)
    ...
    hs_LCoPx(k) = fL_CoPx(Lheel_strikes1(k));
end
...
% Export to Excel 
writetable(SW_Table,'LastSW_Day6_GA2.xls') % change the name to match the trial you are analysing 
